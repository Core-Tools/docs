...
**LLMbench** is a modular, cross-platform CLI-driven framework written in go-lang for executing large language model (LLM) benchmarks. It abstracts the deployment and execution complexity of standard and custom benchmark suites using a unified gRPC-based architecture. The main library can be also used in a go-lang desktop/server applications exposing it's own APIs for benchmarks management
